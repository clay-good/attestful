...
Provides control definitions for all 93 ISO 27001:2022 Annex A controls,
organized by domain with NIST 800-53 and SOC 2 mappings.
...
logger = get_logger("frameworks.iso27001.controls")
...
# Domain names
DOMAIN_NAMES = {
    DOMAIN_ORGANIZATIONAL: "Organizational controls",
    DOMAIN_PEOPLE: "People controls",
    DOMAIN_PHYSICAL: "Physical controls",
    DOMAIN_TECHNOLOGICAL: "Technological controls",
}
...
def get_control(control_id: str) -> ISO27001Control | None:
    """
    ...
    """
    return ISO_27001_CONTROLS.get(control_id)
...
def get_controls_by_domain(domain: str) -> list[ISO27001Control]:
    ...
    Args:
        domain: The domain ID (A.5, A.6, A.7, A.8).
...
def get_all_controls() -> list[ISO27001Control]:
    """
    ...
    """
    return list(ISO_27001_CONTROLS.values())
...
def get_all_domains() -> list[str]:
    ...
    Returns:
        List of domain IDs.
    """
    return [DOMAIN_ORGANIZATIONAL, DOMAIN_PEOPLE, DOMAIN_PHYSICAL, DOMAIN_TECHNOLOGICAL]


def get_control_count_by_domain() -> dict[str, int]:
    """
    Get the count of controls per domain.

    Returns:
        Dictionary mapping domain ID to control count.
    """
    counts: dict[str, int] = {}
...
def get_controls_by_nist_mapping(nist_control_id: str) -> list[ISO27001Control]:
    """
    Get ISO 27001 controls that map to a specific NIST 800-53 control.

    Args:
        nist_control_id: The NIST 800-53 control ID (e.g., "AC-2").

    Returns:
        List of ISO 27001 controls mapping to that NIST control.
    """
    return [
        c for c in ISO_27001_CONTROLS.values()
        if nist_control_id in c.nist_mappings
    ]


def get_controls_by_soc2_mapping(soc2_criterion: str) -> list[ISO27001Control]:
    """
    Get ISO 27001 controls that map to a specific SOC 2 criterion.

    Args:
        soc2_criterion: The SOC 2 criterion ID (e.g., "CC6.1").

    Returns:
        List of ISO 27001 controls mapping to that criterion.
    """
    return [
        c for c in ISO_27001_CONTROLS.values()
        if soc2_criterion in c.soc2_mappings
    ]
